...
var _ datasource.DataSource = &ForwardLookingDelegationDataSource{}

func NewForwardLookingDelegationDataSource() datasource.DataSource {
	return &ForwardLookingDelegationDataSource{}
}

// ForwardLookingDelegationDataSource defines the data source implementation.
type ForwardLookingDelegationDataSource struct {
	client *universalddiclient.APIClient
}

func (d *ForwardLookingDelegationDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "federation_forward_looking_delegations"
}

type ForwardLookingDelegationModelWithFilter struct {
	Filters    types.Map  `tfsdk:"filters"`
	TagFilters types.Map  `tfsdk:"tag_filters"`
	Results    types.List `tfsdk:"results"`
}

func (m *ForwardLookingDelegationModelWithFilter) FlattenResults(ctx context.Context, from []ipamfederation.ForwardLookingDelegation, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
...
func (d *ForwardLookingDelegationDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *ForwardLookingDelegationDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data ForwardLookingDelegationModelWithFilter
	...
	allResults, err := utils.ReadWithPages(func(offset, limit int32) ([]ipamfederation.ForwardLookingDelegation, error) {
